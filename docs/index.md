...
A unified driver for **Claude Code** and **Codex CLI** — runs them inside tmux, exposes them as a Python library and a thin CLI wrapper, and turns their streaming TUI output into a clean event stream that includes *revisions* of previously-rendered lines.
...
flowchart LR
    ...
    class cli,lib face
    class drv_tmux,drv_direct driver
    class ai_tmux,ai_direct agent
```

1. **CLI app** — accepts essentially the same flags as `claude` and `codex` but routes work through a managed tmux session. Multi-line prompts, image attachments, approval prompts (`y`/`Enter`), and slash commands all work — including key presses.
...
Both flavours (Claude Code, Codex) must work. Both their TUI mode and their structured/headless mode must be available.
...
| Driver | When it wins |
|---|---|
| **`tmux`** (primary) | Anything the *interactive TUI* does: approval prompts, slash commands, image paste, `Ctrl+C` to cancel a turn, `/model` to switch model mid-conversation, resuming where you left off and just typing more. The user explicitly asked for this. |
...
The library and CLI accept a `--driver` flag (or `Session(driver="tmux"|"direct")`). Sensible defaults:

- `tmux` if the request needs anything TUI-only (approval flow, slash command, paste a multi-line prompt with bracketed paste).
...
- A stable API surface (`send-keys`, `capture-pane`, `pipe-pane`, control mode `-C`).
...
tmux is the primary backend. Direct subprocess is the fast path for headless work.
...
| **Driver** | How we talk to that backend. `tmux` or `direct`. |
